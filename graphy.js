 ...
 *	
 * 	Copyright (c) 2015 Hydro-c Inc
 * 	Graphy is licensed under the Apache License, Version 2.0
 */
/*
 *  License 2
 * 
...
var dbg = function(s) {
	if(console) console.debug("Graphy-reader: " + s);
};
...
var graphy = {
	...
	**/
	...
	/**
	 * All of the regular expressions in use within graphy.
	 **/
	regexps: {
	...
	init: function(preserveUnlikelyCandidates) {
		...
		// turn it to a boolean value
		preserveUnlikelyCandidates = !!preserveUnlikelyCandidates;

		// backup body content
		if(document.body && !graphy.bodyCache)
			graphy.bodyCache = document.body.innerHTML;
		
		graphy.prepDocument();
		
		/* Build graphy's DOM tree */
		var overlay        = document.createElement("DIV");
		var innerDiv       = document.createElement("DIV");
		var articleTools   = graphy.getArticleTools();
		var articleTitle   = graphy.getArticleTitle();
		var articleContent = graphy.grabArticle(preserveUnlikelyCandidates);
		var articleFooter  = graphy.getArticleFooter();
		...
		**/
		if(graphy.getInnerText(articleContent, false) == "")
		{
			if(!preserveUnlikelyCandidates) {
				document.body.innerHTML = graphy.bodyCache;
				return graphy.init(true);				
			}
			else
			{
